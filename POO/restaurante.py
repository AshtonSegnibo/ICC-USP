...
def exibir_itens(mesa,contas):
    if mesa in contas:
        for n,conta in contas.items():
            if n==mesa:
                print(conta.getItens())
                print(conta.getValores())
    else:
        print('Mesa nao encontrada')
...
def encerrar_conta(mesa,contas):
    if mesa in contas:
        ...
        valores=contas[mesa].getValores()
        valores_contas_encerradas.append(sum(valores))
        contas.pop(mesa)
    else:
        print('Mesa nao encontrada')   
...
while True:
    ...
    if opcao==1:   
        ...
        print(valores_contas_encerradas)
    else:
        pass
